...
class MDOSApp:
    ...
    def _build_report(self):
        ...
        # ── Ligne 2 : toolbar d'exports (toujours visible, peu importe la largeur)
        toolbar = tk.Frame(f, bg=C["bg"])
        toolbar.pack(fill="x", pady=(0, s.px(8)))
        tk.Label(toolbar, text="// EXPORTS :",
                 fg=C["text_dim"], bg=C["bg"],
                 font=self._fonts["tiny"]).pack(side="left", padx=(0, s.px(10)))
        # PDF
        self.green_btn(toolbar, "⬇  PDF",
                       self._export_session_pdf).pack(side="left", padx=s.px(4))
        # JSON
        self.sec_btn(toolbar, "⬇  JSON",
                     self._export_session_json).pack(side="left", padx=s.px(4))
        # NOTION (violet, bien visible)
        b_notion = tk.Button(toolbar, text="◈  NOTION",
                             ...
                             padx=s.px(14), pady=s.px(7), cursor="hand2")
        b_notion.pack(side="left", padx=s.px(4))
...
{"".join(rows)}
    ...
    def _load_notion_env(self):
        """Charge .env depuis cwd, script dir, ou parents. Retourne (token, db_id, source)."""
        token = os.environ.get("NOTION_TOKEN")
        db_id = os.environ.get("NOTION_DATABASE_ID")
        if token and db_id:
            return token, db_id, "env_vars"

        # Essai python-dotenv si présent (gère mieux les .env classiques)
        try:
            from dotenv import load_dotenv as _ld  # type: ignore
            # Cherche un .env vers le haut depuis cwd
            from dotenv import find_dotenv as _fd  # type: ignore
            found = _fd(usecwd=True)
            if found:
                _ld(found, override=False)
                token = token or os.environ.get("NOTION_TOKEN")
                db_id = db_id or os.environ.get("NOTION_DATABASE_ID")
                if token and db_id:
                    return token, db_id, found
        except Exception:
            pass

        # Recherche manuelle : cwd + parents (3 niveaux), script dir + parents, home
        search_dirs = []
        ...
        for d in search_dirs:
            ...
            candidates.append(d / "mdos.env")
        candidates.append(Path.home() / ".mdos.env")
            ...
            try:
                ...
                for ln in envp.read_text(encoding="utf-8", errors="ignore").splitlines():
                    ...
                    if k == "NOTION_TOKEN" and not token:
                        ...
                        db_id = v
                if token and db_id:
                    return token, db_id, str(envp)
            except Exception:
                continue
        return token, db_id, None
    ...
    def _export_session_notion(self):
        if not self.session_log:
            self._notify("⚠  Aucun résultat à exporter", error=True)
            return
        token, db_id, source = self._load_notion_env()
        if not token or not db_id:
            self._notify("⚠  .env non trouvé — saisie manuelle", error=True)
            token, db_id = self._prompt_notion_credentials()
            if not token or not db_id:
                self._notify("⛔ Export Notion annulé", error=True)
                return
            source = "manuel"
        # Petit aperçu de la source pour debug
        src_lbl = source if source else "?"
        if isinstance(src_lbl, str) and len(src_lbl) > 50:
            src_lbl = "…" + src_lbl[-47:]
        self._notify(f"⏳ Envoi Notion ({src_lbl})…")
